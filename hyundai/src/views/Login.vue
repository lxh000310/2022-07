<template>
  <div class="wrapper">
    <div
      ...
      <div style="margin: 20px 0; text-align: center; font-size: 24px;color: #009be2"><b>现代线索平台</b></div>
  ...
  methods: {
    login() {
      this.$refs['userForm'].validate((valid) => {
        ...
        if (valid) {  // 表单校验合法
          this.request.post("https://rw.velo.com.cn/xd/api/user/login", this.user).then(res => {
            console.log(res)
            let a = res.data
            if (res.code === 0) {
